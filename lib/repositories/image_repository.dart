// 파일 위치: lib/repositories/image_repository.dart
...
class ImageRepository {
  ...
  // 서버에서 이미지 불러오기
  Future<List<ImageModel>> fetchImagesFromServer(int pageIndex) async {
    try {
  ...
  Future<void> saveImagesToCache(List<ImageModel> images, int pageIndex) async {
    ...
    for (int i = 0; i < images.length; i++) {
      if (startIndex + i < cachedImages.length) {
        cachedImages[startIndex + i] = images[i];
      } else {
        cachedImages.add(images[i]);
      }
    }
